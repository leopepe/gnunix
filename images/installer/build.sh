...
# the flake and their closures copied in (ADR-024), and the ISO is
...
if [ "$CI_MODE" = "1" ]; then
  ...
  # Nix no longer needs these -- both profiles are built on the runner and
  # copied in. They stay for the chroot(1) calls below, which resolve
  # binaries out of the image and need /proc and /dev to do it.
  ...
  # closures into the image. ADR-024: no nix runs inside the chroot any
  # more, which retires the whole failure class around it -- no channel to
  # subscribe to, no pivot_root sandbox error, no daemon socket to miss.
...
# shellcheck disable=SC2087 # client-side expansion of ${ARCH}/${VER} is intended
ssh $SSH_OPTS "root@$IP" bash <<EOF
set -euo pipefail
export PATH=/nix/var/nix/profiles/system/bin:/nix/var/nix/profiles/installer-build/bin:\$PATH
