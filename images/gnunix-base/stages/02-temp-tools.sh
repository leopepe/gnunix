...
# file — built ahead of the generic loop and NOT part of it.
#
# file's magic.mgc is generated by running a `file` binary, and its build
# refuses to use a host binary of a different version:
#   "Cannot use the installed version of file (5.41) to cross-compile
#    file 5.45. Please install file 5.45 locally first"
# So, per LFS book ch. 6 § file, compile a throwaway NATIVE copy in a
# build/ subdirectory first and point the cross build at it with
# FILE_COMPILE. The host's own `file` version is then irrelevant, which
# also stops this stage depending on whichever version the builder image
# happens to ship.
build_pkg file "$(PKG_VER file)" tar.gz \
