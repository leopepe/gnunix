...
**Superseded:** 2026-09-13 — never merged as the authority. ADR-024 reached the right mechanism but framed it as an amendment carving an exception out of ADR-004, whose objection to flakes it treated as sound-but-inapplicable. The objection was misapplied, not merely narrow, and ADR-004 needed superseding rather than amending. ADR-025 absorbs this ADR's content in full — the flake outputs, the build-on-runner + `nix copy` + `nix-env --set` mechanism, and the `flake.lock` pin — and corrects the boot-persistence claim in the "Boot persistence is unchanged" section below, which overstated what `/etc/profile.d/nix-daemon.sh` puts on `PATH`.
