...
- [ ] **Provision a self-hosted Linux x86_64 runner** labeled `[self-hosted, linux, x64]` (Tart is macOS-only; we need a separate machine or VM). Wire it into the CI matrix.
...
- [ ] **Qemu fallback for tests** — `scripts/run-installer-test.sh` calls `tart` directly. When the Linux x86_64 runner lands (Phase 5), abstract the VM driver behind `scripts/vm-helpers.sh` so tests can target Tart-on-macOS-arm64 or qemu-on-Linux from one entry point.
