...
0. **MemPalace first for searches.** If the question can be answered with a focused excerpt rather than a whole-file load (*"what does ADR-NNN say about X?"*, *"where is Y documented?"*, *"find references to Z"*), query the MemPalace MCP server (`mcp__mempalace__*` tools) before opening any file. The palace is a local index of `docs/` (ADRs, runbooks, architecture) plus the AGENTS.md hierarchy; one query returns a ~600–900-token excerpt instead of pulling whole ADRs into the context window. Fall back to file `Read` only when the answer truly needs whole-file context, when you already know the exact path, or when you need the *current* state rather than the last-synced snapshot. See `AGENTS.md § Searching documentation with MemPalace` for the full guidance.
