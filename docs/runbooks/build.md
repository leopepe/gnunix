...
| chroot | `images/gnunix-base/stages/03-chroot.sh` → `03b-chroot-inner.sh` | full LFS chapter 7 + chapter 8: bison, flex, gperf, pkgconf, perl, libxcrypt, shadow, util-linux, base packages, openssl, sysvinit, eudev, openssh, python, grub | ~19 min |
...
8. **Python is built immediately before GRUB, and exists only for it** — `grub-2.12`'s configure calls `AM_PATH_PYTHON` unconditionally and fails with `configure: error: no suitable Python interpreter found` on a release tarball, not just a git checkout. A comment in `03b-chroot-inner.sh` claimed the opposite for a while; it only looked true because python was built further up the file for `usbutils`. Removing usbutils in #161 took GRUB's interpreter with it (run 34765691034). meson and ninja did not come back — `usbutils` was their only consumer, and anything needing a meson project now builds it in Nix, on the runner.
